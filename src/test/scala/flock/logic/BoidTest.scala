...
class BoidTest extends AnyFlatSpec with Matchers:
  ...
  it should "limit its velocity to maxSpeed in update" in {
    val boid = new Boid(Vector2D(0, 0), Vector2D(100, 0))
    boid.update(0.1, cd) 
    boid.velocity.magnitude() should be <= cd.maxSpeed
  }

  it should "correctly update velocity and position when a force is applied" in {
    val customConstants = cd.copy(
      maxSteeringForce = 0.15,
      maxSpeed = 3.0,
      mass = 1.0
    )
    ...
    boid.update(1.0, customConstants)
